...
export function ReportsList({ refreshTrigger }: ReportsListProps) {
  ...
  const handleExport = async (reportId: number, format: 'markdown' | 'html' | 'pdf') => {
    try {
      let response;
  ...
  if (loading) {
    return <div className="p-8 text-center text-buddy-dark dark:text-gray-400 font-bold uppercase tracking-widest">Loading reports...</div>;
  }
      ...
      {reports.map((report) => (
        ...
        >
          <div className="flex items-start justify-between mb-2">
            <div>
              <h3 className="font-extrabold uppercase text-lg text-buddy-dark dark:text-gray-100 tracking-tight">{report.title}</h3>
          ...
          {selectedReport?.id === report.id && (
            <div className="mt-5 pt-5 border-t-2 border-buddy-dark/10 dark:border-white/10">
              {report.content.hypothesis && (
                <div className="mb-5">
                  <h4 className="font-black uppercase tracking-widest text-sm text-buddy-dark dark:text-buddy-green mb-2">Hypothesis</h4>
                  <div className="text-sm space-y-2 text-buddy-dark dark:text-gray-300 font-medium leading-relaxed">
                    {(report.content.hypothesis.selected_texts || []).map((text, i) => (
                      <p key={i}>{text}</p>
                    ))}
                  </div>
                </div>
              ...
              {report.content.methodology && (
                <div className="mb-5">
                  <h4 className="font-black uppercase tracking-widest text-sm text-buddy-dark dark:text-buddy-green mb-2">Methodology</h4>
                  <div className="text-sm space-y-2 text-buddy-dark dark:text-gray-300 font-medium leading-relaxed">
                    {(report.content.methodology.selected_texts || []).map((text, i) => (
                      <p key={i}>{text}</p>
                    ))}
                  </div>
                </div>
              ...
              {report.content.findings && (
                <div className="mb-5">
                  <h4 className="font-black uppercase tracking-widest text-sm text-buddy-dark dark:text-buddy-green mb-2">Findings</h4>
                  <div className="text-sm space-y-2 text-buddy-dark dark:text-gray-300 font-medium leading-relaxed">
                    {(report.content.findings.selected_texts || []).map((text, i) => (
                      <p key={i}>{text}</p>
                    ))}
                  </div>
                </div>
              )}
              {report.content.conclusions && (
                <div className="mb-5">
                  <h4 className="font-black uppercase tracking-widest text-sm text-buddy-dark dark:text-buddy-green mb-2">Conclusions</h4>
                  <div className="text-sm space-y-2 text-buddy-dark dark:text-gray-300 font-medium leading-relaxed">
                    {(report.content.conclusions.selected_texts || []).map((text, i) => (
                      <p key={i}>{text}</p>
                    ))}
                  </div>
                </div>
              )}
              {report.content.references && report.content.references.length > 0 && (
                <div className="mb-5">
                  <h4 className="font-black uppercase tracking-widest text-sm text-buddy-dark dark:text-buddy-green mb-2">References</h4>
                  <div className="text-sm space-y-2 text-buddy-dark dark:text-gray-300 font-medium leading-relaxed">
                    {report.content.references.map((ref: any, i: number) => (
                      <div key={i}>
                        <span className="font-bold border-b border-buddy-dark/20 dark:border-white/20 pb-0.5">{ref.type}: {ref.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
